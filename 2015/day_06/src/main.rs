...
use anyhow::Result;
// Using HashMap for storing the coordinates and lights
use std::collections::HashMap;

// Instructions for the light display
const INSTRUCTIONS: &str = "turn off 660,55 through 986,197
...
impl Light{
    fn new() -> Light{
        Light{brightness: 0, is_on: false}
    }
...
impl Vector2{
    fn from_str(coordinate_pair: &str) -> Vector2{
        let coordinate_split: Vec<u64> = coordinate_pair.split(',').filter_map(|c| c.parse().ok()).collect();

        Vector2{x: coordinate_split[0], y: coordinate_split[1]}
    }

    fn new(_x: u64, _y: u64) -> Vector2{
        Vector2{x: _x, y: _y}
    }
}
...
fn main() -> Result<()>{
    println!("Calculating light data...");

    // Get the results for both steps
    let (lights_on, total_brightness) = calculate_lights()?;

    println!("{} lights are on in step one. Step two has {} brightness", lights_on, total_brightness);

    Ok(())
}

fn calculate_lights() -> Result<(u64, u32)>{
    // Create a HashSet to store all the lights
    let mut lights: HashMap<Vector2, Light> = HashMap::new();
    ...
    for instruction in INSTRUCTIONS.split('\n'){
        ...
        // Create Vector2 for the start and end
        let begin_vector: Vector2;
        let end_vector: Vector2;

        // Get the execution instruction
        let execute: &str = if instruction_split[0] == "toggle"{
            begin_vector = Vector2::from_str(instruction_split[1]);
            end_vector = Vector2::from_str(instruction_split[3]);
            instruction_split[0]
        } else {
            begin_vector = Vector2::from_str(instruction_split[2]);
            end_vector = Vector2::from_str(instruction_split[4]);
            instruction_split[1]
        };

        // Loop over the coordinates
        for x in begin_vector.x..=end_vector.x{
            for y in begin_vector.y..=end_vector.y{
                // Modify the light if it exists, or insert one
                lights.entry(Vector2::new(x, y))
                .and_modify(|light| {
                    match execute{
                        "on" => light.turn_on(),
                        "off" => light.turn_off(),
    ...
    // Get a count of the lights that are on and brightness
    ...
    // Return the number of lights on and total brightness
    Ok((light_count, total_brightness))
}